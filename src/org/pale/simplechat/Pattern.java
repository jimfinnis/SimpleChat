    ...
    public Node parseNode(Bot b, Node parent, StreamTokenizer tok) throws ParserError, IOException {
        ...
        switch (tok.nextToken()) {
            ...
            case StreamTokenizer.TT_NUMBER:
            	double nn = tok.nval;
            	if(nn - Math.floor(nn) < 0.00001)
					n = new WordNode(this, label, parent, String.valueOf((int)nn));
            	else
					n = new WordNode(this, label, parent, tok.toString());
				break;
            case '[':
                n = new AnyOfNode(b, this, label, parent, tok);
                break;
            case '(':
                n = new SequenceNode(b, this, label, parent, tok);
                break;
            case '^':
                n = new NegateNode(b, this, label, parent, tok);
                break;
            case '.':
                n = new DotNode(this, label, parent, tok);
                break;
            case '?':
                n = new MaybeNode(b, this, label, parent, tok);
                break;
            case '~':
                n = new CategoryNode(b, this, label, parent, tok);
                break;
            case '&':
                n = new StoredPatternNode(b, this, label, parent, tok);
                break;
            default:
        ...
        switch (tok.nextToken()) {
            case '+':
                n = new StarNode(this, n, label, true, parent, tok);
                break;
            case '*':
                n = new StarNode(this, n, label, false, parent, tok);
                break;
            default:
                tok.pushBack();
                break;
