...
http://github.com/jimfinnis/ChatCitizens2[separate project]
...
- `opt` settings - each is a word. Adding an opt setting sets the option
to true. It can be accessed by your application code to determine certain
properties of the entire bot quickly with the `Bot.hasOpt(String)` call.

Here is an example:
----
# This is a test bot!

# just a thing to say we have a greet pattern, perhaps.
